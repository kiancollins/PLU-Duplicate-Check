...
class Product:
    ...
    def __init__(self, code, description, subgroup, supplier_code, season, 
                 supplier_main, cost_price, barcode, vat_rate, rrp, sell_price, stg_price, tarriff, web):
        self.plu_code = code
    ...
    def plu_len(self):
        ...
        if len(str(self.plu_code)) > 15:
            return f"Product: {self.plu_code} has PLU Code length of {len(str(self.plu_code))}. Must be under 15."
    ...
    def desc_len(self):
        ...
        if len(self.description) > 50:
            return(f"Product: {self.plu_code} has description length of {len(str(self.description))}. Must be under 50.")
    ...
    def bad_char(self) -> str:
        ...
        if len(bad_fields) > 0:
            return f"Product: {self.plu_code} contains invalid character(s) {BAD_CHARS} in {bad_fields}"
    ...
    def decimal_format(self):
        ...
        if len(errors) > 0:
            return f"Product: {self.plu_code} has decimal place error in {errors}. Must be 2 decimal places or less"
